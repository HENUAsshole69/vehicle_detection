...
def draw_bounding_box_on_image_array(current_frame_number, image,
  ...
  """
    image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
    is_vehicle_detected, csv_line, update_csv, vehicle = draw_bounding_box_on_image(current_frame_number, image_pil,
                                                                                    ymin, xmin, ymax, xmax, color,
                                                                                    thickness, display_str_list,
                                                                                    use_normalized_coordinates,roi_position)
  ...
  """
    ...
    if (
        ...
        predicted_direction, predicted_speed, is_vehicle_detected, update_csv = speed_prediction.predict_speed(top,
                                                                                                               ...
                                                                                                               roi_position)
        vehicle = Vehicle(color=predicted_color, speed=predicted_speed,
                          image={"left": left / image_temp.shape[1], "right": right / image_temp.shape[1],
                                 "top": top / image_temp.shape[0], "bottom": bottom / image_temp.shape[0]},
  ...
  """
    ...
    for box, color in box_to_color_map.items():
        ...
        if (("car" in display_str_list[0]) or ("truck" in display_str_list[0]) or ("bus" in display_str_list[0])):
            is_vehicle_detected, csv_line, update_csv, vehicle = draw_bounding_box_on_image_array(current_frame_number,
                                                                                                  image,
                                                                                                  ymin,
                                                                                                  xmin,
                                                                                                  ymax,
                                                                                                  ...
                                                                                                  box_to_display_str_map[
                                                                                                      box],
                                                                                                  use_normalized_coordinates=use_normalized_coordinates,roi_position=roi_position)
    ...
    if (1 in is_vehicle_detected):
        ...
        if vehicle is not None:
            vehicle.vehicle_type = class_name
            csv_line_util = class_name + "," + csv_line
    else:
        vehicle = None


    return counter, csv_line_util, vehicle
